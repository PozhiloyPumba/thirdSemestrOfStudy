...
void print_id(struct passwd *user, struct group *group, int *groups, int ngroups)
{
	printf("uid=%d(%s) gid=%d(%s) groups=%d(%s)", user->pw_uid, user->pw_name? user->pw_name: "", group->gr_gid, group->gr_name? group->gr_name: "", group->gr_gid, group->gr_name? group->gr_name: "");
